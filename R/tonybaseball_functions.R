...
#'
#' @export
db_to_bats_opponent <- function(date, team_code) {

  team_info <<- RSQLite::dbGetQuery(db, "SELECT * from teams")

  print(RSQLite::dbGetQuery(db, "SELECT team_FL Team, bats_team_code from teams"))


  query <- paste0("SELECT * FROM yak_24 WHERE Date = :date
                  and (HomeTeamCode = :team_code or AwayTeamCode = :team_code)")

  df <- RSQLite::dbGetQuery(db, query, params = list(date = date, team_code = team_code))   %>%
    dplyr::rename(PitchNo = 1) %>%
    dplyr::mutate(Date = as.Date(Date),
                  Batter = stringr::str_to_title(Batter),
                  Pitcher = stringr::str_to_title(Pitcher),
      ...
      across(c(Batter, Pitcher, Catcher, PitcherId),
             ~stringr::str_replace_all(
               ...
               c('Drew  Stengren' = 'Drew Stengren',
                 ...
                 'Tj Reeves' = 'T.J. Reeves',
                 "Jeissy De La Cruz" = 'Jeissy DeLaCruz',
                 'Andrew Lee' = 'Cheng-Ling "Andrew" Lee',
                 'Giovanni Digiacomoo' = 'Gio DiGiacomo',
                 "Blaze O'saben" = "Blaze O'Saben",
                 "Claude Davenport" = "Clarke Davenport",
                 'Mckenna' = 'McKenna',
                 'Kuckmaner' = 'Kuchmaner',
                 'Wilfrido' = 'Wilfredo',
                 "D'amato" = "D'Amato",
...
pitcher_plot <- function(path_to_file, pitcher_name) {
